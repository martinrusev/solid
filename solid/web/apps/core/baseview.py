...
from solid.web.libs.session import MongoDBSession
from solid.web.template import render as jinja_render
from solid.web.apps.auth.acl import check_permissions
...
class BaseView(tornado.web.RequestHandler):
	...
	def check_xsrf_cookie(self):
		try:
			test = os.environ['SOLID_TEST_ENV']
		except:
	...
	def finish(self, chunk = None):
		if self.session is not None and self.session._delete_cookie:
			self.clear_cookie('solid_session_id')
		elif self.session is not None:
			self.session.refresh() # advance expiry time and save session
			self.set_secure_cookie('solid_session_id', self.session.session_id, expires_days=None, expires=self.session.expires)

		super(BaseView, self).finish(chunk = chunk)


	def _create_session(self):
		session_id = self.get_secure_cookie('solid_session_id')
